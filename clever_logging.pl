...
sub usage {
	die
		...
		"Version: 0.1, 2015-03-02\n" .
		"Author: Valery Mironenko vmironenko\@thumbtack.net\n". 
		"License: GPL\n" .
		"Usage:\n" .
...
sub read_config {
	...
	while (<F>) {
                ...
                s/;+/;/sg;
                s/;*\s*$//sg;
                if ( m/^(.*?[^_]filter=)(.*?)(;(alert_command|repeat_command).*)*$/ ) {
                    my $st='';
                    my $fl='#';
                    my $en='';
                    $st = $1;
	...
	$options{alert_command} ||= '#';
	$options{filter} ||= encode_base64('#','');
	$options{delay} ||= 1.0;
...
sub child_monitoring_process {
	...
	while (1) {
		...
		if (m/^==>\s*(.*?)\s*<==/s) {
			...
			if (m#<clever_loging_command>alert_command=([^;]+);file=([^<]+)</clever_loging_command>#s) { # catch a certain sequence of characters to parse parametrs
				my	$command = $1;
				my	$file = $2;
				s#<clever_loging_command>[^<]*</clever_loging_command>##gs; # Then to wipe
...
sub main {
	...
	message(INFO, "Started clever_logging process $$");
	if ($pid_file) {
	...
	# Create string with names of used files of scoreboard	
	my $list_files=''; 
	foreach my $line_host (@{$config->{HOSTS}}) {
	    my $files='';
	    if ($line_host->{group}) {
		$files='$config->{GROUP}->{'.$line_host->{group}.'}';
	    } else {
		$files='$config->{FILES}';
	    }
	    foreach my $line_file (@{eval ($files)}) {
		my @fls = split /;/ ,$line_file;
		my $fltr=$config->{filter};
		foreach my $i (@fls) {
		    $fltr = $1 if $i =~ m/^filter=(.*)/ ;
		}
		$list_files .= md5_hex("$line_host->{orig}|$fls[0];$fltr").'.txt;';
	    }
	}

	# Define and remove no needed files of scoreboard 
	opendir(DIR, $config->{scoreboard}) or die "can't opendir $config->{scoreboard}: $!";
	while (defined(my $fls = readdir(DIR))) {
	    next if $fls =~ /^\.\.?$/; 
	    unlink("$config->{scoreboard}/$fls") or die "Can't delete $fls:  $!\n"    if "$list_files" !~ "$fls";
	}
	closedir(DIR);
...
sub DATA_main {
	my ($p_wildcards, $p_scoreboard, $p_delay, $p_server_id, $p_my_host, $filter, $repeat_command_timeout, $alert_command, $sleep_send_line) = @ARGV;
	$my_host = $p_my_host;
	...
	my $lock_file = "/tmp/clever_logging.$p_server_id.lock"; # /tmp availables to all users
	...
	print LOCK $$ . "\n";
	# Do not close LOCK!
	tail_follow($wildcards, $scoreboard_hash, $p_delay, $filter, $repeat_command_timeout, $alert_command, $sleep_send_line);
}
sub tail_follow {
	my ($wildcards, $scoreboard_hash, $delay, $filter, $repeat_command_timeout, $alert_command, $sleep_send_line) = @_;
	my $last_ping = 0;
	my %time_sb = ();
	my %time_cmd = ();
	while (1) {
		...
		foreach my $file_ (@files) {
			...
			my $tail = $file . ";" .$fltr; # This is needed to create unique name of a scoreboard file for the pair names of file and filter.
			...
			if ($inode == $sb->{inode}) {
				$sb->{pos} = 0 if $sb->{pos} > $stat[7]; # If the file was cleaned.
				seek(F, $sb->{pos}, 0);
			} else {
			...
			while (<F>) {
				...
				if ($fltr_dec ne '#') {
					if ( eval("\$_ =~ m$fltr_dec ") ) { 
						if ( $command ne "#" ) { #There is the command in the config file
							if ($time_cmd{$tail} < ( time() - $timeout )){
								$time_cmd{$tail} = time();
								$notice = "<clever_loging_command>alert_command=".$command.";file=".$file."</clever_loging_command>"; # pack parametrs into the message
							}
						}
					} else {
						$sb->{pos} += length;
					...
					select(undef, undef, undef, $sleep_send_line) if $sleep_send_line != 0; # Set speed limit on the transmission 
				}
				if (!$sb_sent) {
					print_scoreboard_item($sb);
					$sb_sent = 1;
